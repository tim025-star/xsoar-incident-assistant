...
try {
  ...
  await searchPage.route("https://xsoar.example.test/**", (route) => route.fulfill({
    contentType: "text/html",
    body: `<header><span class="header-search"><input autocomplete="off" placeholder="Search in Incidents" type="text" class="xsoar-input search-input header-search-input"></span></header><main id="incidents-page"><div class="react-select-dropdown-multiple__input"><input type="text"></div><label>Incident query<textarea aria-label="Incident search query">-status:closed -category:job</textarea></label><div role="grid" aria-rowcount="0"></div><div class="table-paging-message"></div></main><script>document.querySelector("textarea").addEventListener("keydown", (event) => { if (event.key !== "Enter") return; event.preventDefault(); document.querySelector("[role=grid]").innerHTML = '<a href="/Custom/GenericLayout/4199">4199</a>'; document.querySelector("[role=grid]").setAttribute("aria-rowcount", "1"); document.querySelector(".table-paging-message").textContent = "1-1 of 1"; });</script>`
  }));
