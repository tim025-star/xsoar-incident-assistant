...
Laya incident routing is opt-in on Home after installing the reviewed checkpoint. Activation checks the installed checkpoint identity and local runtime. When enabled, Playwright-collected complete alert JSON is bounded and stripped of credential-bearing keys before local Laya processing. The normal run-status panel shows Laya stages and progress. Only selected matches from complete processing replace configured source fields in the existing response template; tentative, incomplete, and missing matches keep the configured fields. The finished response lists Laya-supplied field names and source pointers separately for analyst review, and tentative matches are visibly identified but never applied. Qwen enrichment is unchanged. Training code, datasets, and checkpoint-management controls are not part of the core application or installer. Upgrading does not delete existing datasets, multilingual downloads, or custom checkpoints already stored on disk.
...
XSOAR routes vary by deployment. Before operational use, confirm the configured incident route, URL template, incident list path, and search parameter against synthetic incidents. The Historic search query in Configuration is editable; its default submits exact `rawName` and `tenantname` values plus a three-month creation window through XSOAR's main incidents search input. The `{incidentName}` and `{tenantName}` placeholders are replaced with quoted, escaped values from the selected incident. Each candidate is checked against the selected ticket's tenant and incident name before its resolution is included. Automation stops or omits the candidate if navigation leaves the configured HTTPS origin or an incident path does not match.
