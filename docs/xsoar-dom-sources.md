...
- The default query uses `rawName` from the incident header name (`.header-inv-title`), not the separate `Rule Name` field, and `tenantname` from the incident's `Tenant Name` field. Template and JSON modes support `{incidentName}` and `{tenantName}` placeholders, inserted as quoted, escaped values. JavaScript mode receives those values and `ticketId` in a limited `incident` object and provides `quote(value)` for safely inserting them. The `Type` column is a separate value and is not used by the default query.
