...
The pilot writes `<output>.pilot-state.pt` atomically after every completed epoch, including epochs below the accuracy gate. This recovery file preserves trainable parameters at training precision, all model buffers, optimizer, fixed scheduler, scaler, epoch/step counters, elapsed training time, and Python/Torch/CUDA RNG state. Frozen parameters are reloaded from the hash-verified base, keeping head-only recovery smaller than a full-model copy. Non-finite loss or gradients stop training; non-finite recovery values cannot replace the last valid state. The file is always `pilotOnly` and non-promotable; it is separate from the final inference checkpoint. To resume, repeat the same command with `--resume-state <output>.pilot-state.pt`. Keep the original run ID, output, scope, device, epoch cap, batch settings, and exact configuration/data/base/reviewer artifacts. Recovery rejects changed bindings, parameter/buffer keys or precision, and trainer/compiler/runtime versions, and preserves the original schedule rather than starting a new one. Only pilot runs support recovery. A run interrupted before its first completed epoch has no recovery state.
