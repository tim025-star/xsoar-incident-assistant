...
- Optional Laya mapping runs in a persistent local child process with all Hugging Face/Transformers network modes disabled. It receives complete bounded alert JSON unchanged, does not write request content or mapped values to logs, and may load checkpoints only from its application data directory. Candidate selection is performed by Laya rather than deterministic key or value heuristics. Pointer existence and canonical value validity are checked after selection, with visible deterministic fallback.
- Laya clients do not install or invoke a system Python, pip, PowerShell, WinGet, service, or PATH entry. Self-contained application-local archives and model files must match the reviewed manifest's byte lengths and SHA-256 hashes. Archives are listed before extraction, unsafe absolute or parent-traversal paths are rejected, and replacement uses staging and rollback. An IT-provided offline asset directory is subject to the same verification as GitHub downloads.
- Laya datasets are written only after an explicit analyst action, use atomic per-example files outside `config.json`, and can be inspected, exported, removed, or deleted. Dataset mutation, training, checkpoint import, model installation, and response generation share an operation lock. Imported checkpoints allow only bounded JSON, tokenizer data, and `safetensors`; pickle and executable files are rejected.
